...
class DetectionFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnDetection.setOnCheckedChangeListener { _, isChecked ->
            ...
            (currentViewModel as DetectionViewModel).btnDetectionOnClick(isChecked)
    ...
    private fun initConfusionMatrix() {
        ...
        //第0行第0列为空
        val textViewZero = TextView(this.context)
        textViewZero.text = ""
        viewModel.confusionMatrix[0][0] = textViewZero
        //第1-8行且第1-8列为数字
        for (i in 1..8) {
            for (j in 1..8) {
        ...
        for (i in 1..8) {
            ...
            textViewRow.setTextColor(ContextCompat.getColor(context!!, R.color.colorAccent))
            ...
            textViewColumn.setTextColor(ContextCompat.getColor(context!!, R.color.colorAccent))
